	...
	*/
	function datamapper(input, settings) {
		...
		if (settings.api == "openweathermap") {
			...
			out.today.sunrise = moment(input[0].sys.sunrise * 1000).format("HH:mm");
			out.today.sunset = moment(input[0].sys.sunset * 1000).format("HH:mm");

			out.today.day = moment().format("dddd").charAt(0).toUpperCase() + moment().format("dddd").slice(1);;

			out.forecast = [];
			for (var i = 0; i < settings.forecast; i++) {
				var forecast = {};
				forecast.day = moment(input[1].list[i].dt_txt).format("HH:mm")
				forecast.code = input[1].list[i].weather[0].id;
				forecast.desc = input[1].list[i].weather[0].description.capitalize();
				forecast.temp = { max: Math.round(input[1].list[i].main.temp_max), min: Math.round(input[1].list[i].main.temp_min) }
